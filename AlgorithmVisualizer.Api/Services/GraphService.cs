...
public class GraphService
{
    ...
    // Time: O(V^3)
    // Space: O(V^2)
    public List<AlgorithmStep> FloydWarshall(GraphRequest req)
    {
        int n = req.NodeCount;
        var dist = new int[n, n];
        for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            dist[i, j] = i == j ? 0 : 99999;
        foreach (var e in req.Edges)
            dist[e[0], e[1]] = e.Length > 2 ? e[2] : 1;
        ...
        steps.Add(
            ...
            {
                StepNumber = step++,
                Array = GetRow(dist, 0, n),
                Description = "Floyd-Warshall initialized",
            }
        );
        ...
        for (int k = 0; k < n; k++)
        {
            ...
            steps.Add(
                ...
                {
                    StepNumber = step++,
                    Array = GetRow(dist, k, n),
                    Description =
                        $"After intermediate node {k}: row {k} = [{string.Join(", ", GetRow(dist, k, n).Select(d => d >= 99999 ? "∞" : d.ToString()))}]",
                    HighlightIndices = [k],
                }
            );
        }

        var allDist = new int[n];
        for (int i = 0; i < n; i++)
            allDist[i] = dist[0, i] >= 99999 ? -1 : dist[0, i];
        steps.Add(
            new AlgorithmStep
            {
                StepNumber = step,
                Array = allDist,
                Description = "Floyd-Warshall complete (showing distances from node 0)",
    ...
    private static int[] GetRow(int[,] dist, int row, int n)
    {
        var r = new int[n];
        for (int j = 0; j < n; j++)
            r[j] = dist[row, j] >= 99999 ? -1 : dist[row, j];
        return r;
    }
