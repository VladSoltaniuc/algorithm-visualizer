...
public class BacktrackingService
{
    ...
    // Time: O(8^(n^2))
    // Space: O(n^2)
    public List<AlgorithmStep> KnightsTour(int n)
    {
        if (n < 5)
            throw new ArgumentException("Board size must be at least 5 for Knight's Tour.");
        ...
        steps.Add(
            new AlgorithmStep
            {
                StepNumber = step++,
                Array = (int[])board.Clone(),
                Description = $"Knight's Tour on {n}×{n} board",
            }
        );

        if (KnightHelper(board, 0, 0, 1, n, steps, ref step))
            steps.Add(
                new AlgorithmStep
                {
                    StepNumber = step,
                    ...
                    SortedIndices = Enumerable.Range(0, n * n).ToArray(),
                }
            );
        else
            steps.Add(
                new AlgorithmStep
                {
                    StepNumber = step,
                    Array = (int[])board.Clone(),
                    Description = "No complete tour found",
                }
            );
        return steps;
    }

    private static readonly int[] KDr = [-2, -1, 1, 2, 2, 1, -1, -2];
    private static readonly int[] KDc = [1, 2, 2, 1, -1, -2, -2, -1];

    private static bool KnightHelper(
        int[] board,
        int r,
        int c,
        int moveNum,
    ...
    )
    {
        ...
        var moves = new List<(int nr, int nc, int deg)>();
        for (int d = 0; d < 8; d++)
        {
            int nr = r + KDr[d],
                nc = c + KDc[d];
            if (nr >= 0 && nr < n && nc >= 0 && nc < n && board[nr * n + nc] == -1)
            {
                int deg = 0;
                for (int d2 = 0; d2 < 8; d2++)
                {
                    int nr2 = nr + KDr[d2],
                        nc2 = nc + KDc[d2];
                    if (nr2 >= 0 && nr2 < n && nc2 >= 0 && nc2 < n && board[nr2 * n + nc2] == -1)
                        deg++;
                }
                moves.Add((nr, nc, deg));
            }
        ...
        foreach (var (nr, nc, _) in moves.OrderBy(m => m.deg))
        {
            board[nr * n + nc] = moveNum;
            if (step < 200) // Limit visualization steps
                steps.Add(
                    new AlgorithmStep
                    {
                        StepNumber = step++,
                        Array = (int[])board.Clone(),
                        Description = $"Move {moveNum}: knight to ({nr},{nc})",
                        HighlightIndices = [nr * n + nc],
                    }
                );
            if (KnightHelper(board, nr, nc, moveNum + 1, n, steps, ref step))
                return true;
            board[nr * n + nc] = -1;
        }
        return false;
    ...
    // 10. Graph Coloring
    // Time: O(k^V) where k = colors, V = vertices
    // Space: O(V)
    public List<AlgorithmStep> GraphColoring(GraphRequest req, int numColors)
    {
        int n = req.NodeCount;
        ...
        steps.Add(
            ...
            {
                StepNumber = step++,
                Array = (int[])colors.Clone(),
                Description = $"Graph coloring: {n} nodes, {numColors} colors",
            }
        );

        if (ColorHelper(adj, colors, 0, n, numColors, steps, ref step))
            steps.Add(
                new AlgorithmStep
                {
                    StepNumber = step,
                    Array = (int[])colors.Clone(),
                    Description = $"Coloring found: [{string.Join(", ", colors)}]",
                    SortedIndices = Enumerable.Range(0, n).ToArray(),
                }
            );
        else
            steps.Add(
                new AlgorithmStep
                {
                    StepNumber = step,
                    Array = (int[])colors.Clone(),
                    Description = "No valid coloring exists",
                }
            );
        return steps;
    }

    private static bool ColorHelper(
        List<List<int>> adj,
        int[] colors,
        int node,
        int n,
    ...
    )
    {
        ...
        {
            if (adj[node].All(nb => colors[nb] != c))
            {
                colors[node] = c;
                steps.Add(
                    new AlgorithmStep
                    {
                        StepNumber = step++,
                        Array = (int[])colors.Clone(),
                        Description = $"Color node {node} with color {c}",
                        HighlightIndices = [node],
                    }
                );
                if (ColorHelper(adj, colors, node + 1, n, numColors, steps, ref step))
                    return true;
                colors[node] = 0;
                steps.Add(
                    new AlgorithmStep
                    {
                        StepNumber = step++,
                        Array = (int[])colors.Clone(),
                        Description = $"Backtrack node {node}",
                        HighlightIndices = [node],
                    }
                );
            }
