...
export const arrayConfig: AlgorithmConfig[] = [
  ...
  },
  {
    ...
    cons: [
      'Often requires sorted input depending on the problem, sorting first adds O(n log n)',
...
export const stringConfig: AlgorithmConfig[] = [
  ...
  },
  {
    ...
    cons: [
      'O(n) worst case with both bad-character and good-suffix tables, but pathological inputs expose it on simplified implementations',
...
export const treeConfig: AlgorithmConfig[] = [
  ...
  },
  {
    ...
    pros: [
      'O(h) time, follows a single root-to-LCA path using BST value comparisons',
      'O(1) extra space with iterative implementation',
      'Leverages BST ordering to skip entire subtrees, no need to visit the whole tree',
    ],
    cons: [
      'O(n) time worst case for skewed trees',
      'Only works on BSTs, general binary trees require an O(n) algorithm',
    ],
  ...
  },
  {
    ...
    description: 'At every node in a valid BST, every value in the left subtree must be strictly less than the current node, and every value in the right subtree must be strictly greater. Validate BST enforces this by passing a valid range (min, max) down the tree. Each node checks whether its value falls within its allowed range, then narrows that range for its children: the left child inherits the current node\'s value as its new upper bound, the right child inherits it as the new lower bound.',
...
export const graphConfig: AlgorithmConfig[] = [
  ...
  },
  {
    ...
    pros: [
      'O(V²) time with simple array scan (shown here), easy to implement',
      'Doesn\'t require sorting all edges upfront, grows the MST incrementally',
      'O(V) space for the key and inMst arrays',
    ],
    cons: [
...
export const dpConfig: AlgorithmConfig[] = [
  ...
  },
  {
    ...
    ytTutorial: 'https://www.youtube.com/watch?v=MrPa5EFcDCU',
  },
  {
...
export const nrTheoryConfig: AlgorithmConfig[] = [
  {
    ...
    ytTutorial: 'https://www.youtube.com/watch?v=H_NCHm3wAMI',
  },
];
